...
class AcessoViewController: ClubMesaViewController {
    ...
    private func confirmar() {
        if let nome = nomeTextField.text, let celular = celularTextField.text, let dataNascimento = nascimentoDateField.date {
            let pais = celular.substring(with: 1..<3)
            let prefixo = celular.substring(with: 5..<7)
            let numero = celular.substring(from: 9).replacingOccurrences(of: "-", with: "")
            let dispositivo = Dispositivo(uuid: Device.uuid, nome: nome, pais: pais, numero: numero, prefixo: prefixo, dataNascimento: dataNascimento)
