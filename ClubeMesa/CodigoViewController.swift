...
class CodigoViewController: ClubMesaViewController {
    ...
    private func validar() {
        if isValid, let dispositivo = Dispositivo.current {
            ...
            let observable = DispositivoService.Async.verificar(pais: dispositivo.pais, prefixo: dispositivo.prefixo, numero: dispositivo.numero, codigo: codigo)
            prepare(for: observable)
                .subscribe(
                    onNext: { model in
                        if model.status == .verificado {
                            self.presentSceneBemVindo()
                        } else {
    ...
    private func reenviar() {
        if let dispositivo = Dispositivo.current {
            showActivityIndicator()
            let observable = DispositivoService.Async.reenviar(pais: dispositivo.pais, prefixo: dispositivo.prefixo, numero: dispositivo.numero)
            prepare(for: observable)
                .subscribe(
                    onError: { error in
                        self.hideActivityIndicator()
    ...
    private func presentSceneBemVindo() {
        if let controller = self.storyboard?.instantiateViewController(withIdentifier: "Scene.BemVindo") {
