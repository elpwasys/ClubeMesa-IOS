...
class WebViewController: DrawerViewController {
    ...
    var link: String?
    static let linkKey = "\(AppDelegate.self).linkKey"
    ...
    private func carregar() {
        if self.link == nil {
            if let link = UserDefaults.standard.value(forKey: WebViewController.linkKey) as? String {
                self.link = link
                UserDefaults.standard.removeObject(forKey: WebViewController.linkKey)
            } else {
                self.link = Config.link(forKey: "ClubConsulta")
            }
        }
        if let link = self.link {
            if let url = URL(string: "\(Config.mobileURL)/\(link)") {
                var request = URLRequest(url: url)
                request.addValue("clear", forHTTPHeaderField: Device.Header.deviceHistory.rawValue)
...
extension WebViewController {
    ...
    fileprivate func openSceneVoucher(_ codigo: String) {
        showActivityIndicator()
        let observable = VoucherService.Async.buscar(codigo: codigo)
        prepare(for: observable)
            .subscribe(
                onNext: { model in
                    self.decideScene(model)
                },
                onError: { error in
    ...
    fileprivate func decideScene(_ model: VoucherModel) {
        let controller = storyboard?.instantiateViewController(withIdentifier: "Scene.VoucherVisualizacao") as! VoucherVisualizacaoViewController
        controller.voucher = model
    ...
    func onPayment(success: Bool, id: Int) {
        webView.stringByEvaluatingJavaScript(from: "Device.onPayment(\(success), '\(id)')")
    }
}
    ...
    func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
        ...
        if let scheme = url.scheme {
            ...
            case Scheme.dataUpdate.rawValue:
                ...
                return false
            case Scheme.voucher.rawValue:
                if let codigo = self.getArgs(scheme) {
                    self.openSceneVoucher(codigo)
                }
                return false
